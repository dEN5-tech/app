...
class Server(Thread):
    ...
    def __init__(self, my_OneServer=OneServer, port=80,
        ...
        self._go_on = True
        self.oneServers = []
        self.max_connection = 4 * 32
        self.showing_max_warning = False
    ...
    def __handleQueue(self, intent):
        if isinstance(intent, DeregisterOneServer):
    ...
    def run(self):
        ...
        while self._go_on:
            if len(self.oneServers) >= self.getMaxConnection():
                if not self.showing_max_warning:
                    log('Max connection reached.')
                    self.showing_max_warning = True
            else:
                if self.showing_max_warning:
